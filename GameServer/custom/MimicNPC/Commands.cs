...
namespace DOL.GS.Scripts
{
    ...
    "&mbot",
    ePrivLevel.Player,
    "/mbot <name> <realm> <race> <class> <level> <gender> - Create a custom mimic with specific attributes.")]
    public class MimicBotCommandHandler : AbstractCommandHandler, ICommandHandler
    {
        public void OnCommand(GameClient client, string[] args)
        {
            GamePlayer player = client.Player;
            ...
            if (args.Length < 7)
            {
                player.Out.SendMessage("Usage: /mbot <name> <realm> <race> <class> <level> <gender>", eChatType.CT_Say, eChatLoc.CL_ChatWindow);
                player.Out.SendMessage("Example: /mbot Gareth alb briton armsman 50 male", eChatType.CT_Say, eChatLoc.CL_ChatWindow);
                return;
            }
            ...
            switch (args[2].ToLower())
            {
                ...
                case "3":
                    ...
                    break;
                default:
                    player.Out.SendMessage("Invalid realm: " + args[2] + ". Use alb/mid/hib.", eChatType.CT_Say, eChatLoc.CL_ChatWindow);
                    return;
            }

            // Parse race
            if (!Enum.TryParse<eRace>(args[3], true, out eRace race))
            {
                player.Out.SendMessage("Invalid race: " + args[3], eChatType.CT_Say, eChatLoc.CL_ChatWindow);
                return;
            }

            // Parse class
            if (!Enum.TryParse<eMimicClass>(args[4], true, out eMimicClass mclass))
            {
                player.Out.SendMessage("Invalid class: " + args[4], eChatType.CT_Say, eChatLoc.CL_ChatWindow);
                return;
            }

            // Validate class belongs to realm
            if (!GlobalConstants.STARTING_CLASSES_DICT.ContainsKey(realm) ||
                !GlobalConstants.STARTING_CLASSES_DICT[realm].Contains((eCharacterClass)(int)mclass))
            {
                player.Out.SendMessage(mclass + " is not a valid class for " + realm, eChatType.CT_Say, eChatLoc.CL_ChatWindow);
                return;
            }

            // Validate race is eligible for class
            ICharacterClass charClass = ScriptMgr.FindCharacterClass((int)mclass);
            if (charClass == null)
            {
                player.Out.SendMessage("Could not find character class for " + mclass, eChatType.CT_Say, eChatLoc.CL_ChatWindow);
                return;
            }

            bool raceEligible = false;
            foreach (PlayerRace pr in charClass.EligibleRaces)
            {
                if (pr.ID == race)
                {
                    raceEligible = true;
            ...
            if (!raceEligible)
            {
                player.Out.SendMessage(race + " is not a valid race for " + mclass, eChatType.CT_Say, eChatLoc.CL_ChatWindow);
                return;
            }

            // Parse level
            if (!byte.TryParse(args[5], out byte level) || level < 1 || level > 50)
            {
                player.Out.SendMessage("Level must be between 1 and 50.", eChatType.CT_Say, eChatLoc.CL_ChatWindow);
                return;
            }
            ...
            switch (args[6].ToLower())
            {
                ...
                case "female":
                    ...
                    break;
                default:
                    player.Out.SendMessage("Invalid gender: " + args[6] + ". Use male or female.", eChatType.CT_Say, eChatLoc.CL_ChatWindow);
                    return;
            }

            // Validate race gender constraints
            if (GlobalConstants.RACE_GENDER_CONSTRAINTS_DICT.TryGetValue(race, out eGender raceGenderConstraint))
            {
                if (gender != raceGenderConstraint)
                {
                    player.Out.SendMessage(race + " must be " + raceGenderConstraint, eChatType.CT_Say, eChatLoc.CL_ChatWindow);
                    return;
                }
            }

            // Validate class gender constraints
            if (GlobalConstants.CLASS_GENDER_CONSTRAINTS_DICT.TryGetValue((eCharacterClass)(int)mclass, out eGender classGenderConstraint))
            {
                if (gender != classGenderConstraint)
                {
                    player.Out.SendMessage(mclass + " must be " + classGenderConstraint, eChatType.CT_Say, eChatLoc.CL_ChatWindow);
                    return;
                }
            }
            ...
            // Spawn mimic
            MimicNPC mimic = MimicManager.GetMimic(mclass, level, race, gender, name);
            MimicManager.AddMimicToWorld(mimic, position, player.CurrentRegionID);

            // Auto-invite to group
            if (GameServer.ServerRules.IsSameRealm(player, mimic, true))
            {
                if (player.Group == null)
                {
                    player.Group = new Group(player);
